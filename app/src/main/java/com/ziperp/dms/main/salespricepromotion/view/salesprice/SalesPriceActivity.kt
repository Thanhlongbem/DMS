...
class SalesPriceActivity : BaseActivity() {

    private var filterDialog: FilterDialogFragment? = null
    lateinit var adapter: SalesPriceAdapter
    ...
    override fun initView() {
        ...
        viewModel.getSalesPriceList()
        viewModel.salesPriceData.observe(this, Observer {
            when (it.status) {
                Status.SUCCESS -> {
    ...
    private fun initList() {
        adapter = SalesPriceAdapter()

        adapter.onClickListener = { position ->
            val intent = Intent(this, SalesPriceInfoActivity::class.java)
            intent.putExtra(EXTRA_PRICE_LIST_CODE, adapter.data[position].priceListCd)
    ...
    companion object {
        const val EXTRA_PRICE_LIST_CODE = "EXTRA_PRICE_LIST_CODE"
    }
}