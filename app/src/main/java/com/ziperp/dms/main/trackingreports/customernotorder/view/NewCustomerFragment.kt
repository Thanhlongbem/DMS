...
class NewCustomerFragment(val response: NewCustomerResponse) : BaseFragment() {
    ...
    override fun initView() {
        ...
        scrollListener.setOnLoadMoreListener(object : OnLoadMoreListener {
            override fun onLoadMore() {
                if (viewModel.newCustomerPagingParam.hasNext()) {
                    viewModel.getNewCustomerList(true)
                }
            }
        })
    ...
    private fun dataObserver() {
        viewModel.newCustomerListData.observe(this, Observer {
            when (it.status) {
                Status.SUCCESS -> {
